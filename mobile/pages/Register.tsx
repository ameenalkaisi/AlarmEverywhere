...
const Register: React.FC<NativeStackScreenProps<RootStackParamList, 'Register'>> = ({ navigation }) => {
	...
	const handleSignup = async () => {
		...
		if (!email || !password || !name || name.length == 0 || email.length == 0 || password.length == 0) {
			setPasswordText("Name, e-mail, or password is/are empty");
			return;
		}
	...
	const signUpUser = async (name: string, email: string, password: string) => {
		return await axios.post(BACKEND_URL + '/users/add', {
			name,
			email,
			password,
		})
